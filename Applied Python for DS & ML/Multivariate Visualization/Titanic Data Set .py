...
# 2-d Bar Plot
# 1st & 2nd categorical variable: 'Survival_Status' and 'Pclass'
# Quantitavtive Variable: 'Age'

ax = sb.barplot(data = df, x = 'Survival_Status', y = 'Age', hue = 'Pclass')
ax.legend(loc = 8, ncol = 3, framealpha = 1, title = 'Pclass')
plt.title('Relationship Between Two Categorical Variables And one Quantitative Variable')
...
# Plotting Scatter Plot using color Encoding for two quantitative and one categorical variable 
# Quantitative variables: 'Fare' And 'Age', Categorical variables: 'Pclass'

g = sb.FacetGrid(data = df, hue = 'Pclass',  height = 5, palette = 'colorblind')
...
# Plotting Scatter plot using color Encoding for three quantitative variable 
# Using cmap parameter and colorbar 
# Quantitative variables: 'Fare','Age' And 'SibSp'

plt.scatter(data = df, x = 'Age', y = 'Fare', c = 'SibSp', cmap = 'mako')
plt.colorbar(label='SibSp')
plt.xlabel('Age')
plt.ylabel('Fare')
plt.title('Scatter Plot Representing Relationship between Age, Fare And SibSp');


# In[56]:


# Plotting of two quantitative and one categorical variable using encodings by shape
# Quantitative variables: 'Fare' And 'Age', Categorical variables: 'Survival_Status'
 
cat_markers = [['Survived', 's'],
               ['Did Not Survived', 'o']]

for cat, marker in cat_markers:
    df_cat = df[df['Survival_Status'] == cat]
    plt.scatter(data = df_cat, x = 'Fare', y = 'Age', marker = marker)
...
# PairGrid plot showing relationship between two categorical and two quantitative variable
# Qantitative: 'Fare' And 'Age', Categorical: 'Pclass' And 'Survival_Status'

g = sb.FacetGrid(data = df, col = 'Pclass', row = 'Survival_Status', height = 2.7,
                margin_titles = True)
...
# Delete rows with NaN values in columns 'Age' and 'Fare'.
df_clean = df.dropna(subset=['Age', 'Fare'])

# Plot a 2D histogram between the quantitative variables 'Age', 'Fare' and 'Pclass'.
plt.hist2d(x=df_clean['Age'], y=df_clean['Fare'], bins=10, weights=df_clean['Pclass'])
plt.colorbar()
...
# Plotting Point plot between two categorical variable and one quantitative variable
# Categorical Variables: 'Survival_Status' And 'Pclass'
# Quantitative Varaible: 'Age'

ax = sb.pointplot(data = df, x = 'Survival_Status', y = 'Age', hue = 'Pclass',dodge = 0.3,  linestyles = "")
...
sb.palplot(sb.color_palette('mako', 9))
...
sb.palplot(sb.color_palette('dark',n_colors=9))
