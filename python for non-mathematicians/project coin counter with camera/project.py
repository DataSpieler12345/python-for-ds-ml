...
# Definición de alineación del área de trabajo
def alignment(image, width, height):
    ...
    # Alineación de contornos y jerarquía
...
while True:
    ...
    image_A6 = alignment(camera, width=480, height=677)  # Área de trabajo
    
    if image_A6 is not None:
        image_gray = cv2.cvtColor(image_A6, cv2.COLOR_BGR2GRAY)
        blur = cv2.GaussianBlur(image_gray, (5, 5), 1)  # Desenfoque de la imagen con un patrón gaussiano
